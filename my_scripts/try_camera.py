...
def watch_camera(env, camera, gripper):
    """Thread function to continuously capture and display camera feed."""
    while True:
        try:
            # Capture RGB image
...
def _main(use_graphics=True):
    ...
    # Start a thread to watch the camera feed
    print("Start a thread to watch the camera feed")
    camera_thread = threading.Thread(target=watch_camera, args=(env, camera, gripper))
    camera_thread.start()
    ...
    env.step()
    # Wait for the camera thread to finish before exiting
    camera_thread.join()
