...
class Pipeline:
    ...
    # def movement_from_A_to_landmark(self, fixed_point, final_point):
    #     """
    ...
    #     distance = self.compute_distance_3d(fixed_point, trans_point)
    #     print(f"Move back by {distance * scaling_fac} units")
    #     self.robot.MoveBack(distance * scaling_fac, 0.5)
    #     self.env.step(500)
    ...
    #     distance = self.compute_distance_3d(robot_position, trans_point)
    #     print(f"Move forward by {distance * scaling_fac} units")
    #     self.robot.MoveForward(distance * scaling_fac, 0.5)
    #     self.env.step(700)
    ...
    #     # Move to final point
    #     robot_position = self.robot.data["position"]
    #     distance = self.compute_distance_3d(robot_position, final_point)
    #     print(f"Move forward by {distance * scaling_fac} units")
    #     self.robot.MoveForward(distance * scaling_fac, 0.5)
    #     self.env.step(500)
    #     # env.step(200)
    ...
    #     # Turn left
    #     print(f"Turn left")
    #     # self.robot.TurnLeft(85, 1)
    #     self.robot.TurnLeft(90, 1)
    #     self.env.step(300)
    #     #Position to pick the sponge
    #     print(f"Move to pick sponge")
    #     self.robot.MoveForward(0.25, 0.2)
    #     self.env.step(300)
    ...
    #     # lift_gripper_position = [sponge_position[0] - 0.03, sponge_position[1] + 0.22, sponge_position[2] - 0.9]
    #     lift_gripper_position = [sponge_position[0] - 0.03, sponge_position[1] + 0.3, sponge_position[2] - 0.9]
    ...
    #         speed_based=False,
    #     )
    #     robot_position_prev = self.robot.data["position"]
    #     print(f"Robot position: {robot_position_prev}")
    #     self.env.step(200)
        
    #     # Define start and target positions
    #     start_position = self.robot.data["position"]
    #     # target_position = [0.29251301288604736, 0.2508874237537384, 1.3711309432983398+0.5]
    #     # target_position = [0.29251301288604736, 0.2508874237537384, 1.3711309432983398+0.4]
    #     # !!! The base is not in parallel with bed frame

    #     self.generate_landmarks()
    #     target_position = [self.target_x_nose, 0.2508874237537384, self.target_z]
    #     # target_position = [0.29251301288604736, 0.2508874237537384, -0.3711309432983398+0.3]
    #     # target_position = [0.29251301288604736, 0.2508874237537384, 1.3711309432983398+0.2] # The position is different with the same code

    #     # Move the robot to the target position
    #     self.movement_from_A_to_landmark(start_position, target_position)
    ...
    def get_manikin_coordinates_depth(self):
        ...
        # self.nose_sholder = self.target_nose[0] - self.target_left_shoulder[0]
        # print("nose sholder ", self.nose_sholder)

        # self.depth_nose = self.get_depth(body_part="NOSE")
        # self.depth_right_shoulder = self.get_depth(body_part="RIGHT_SHOULDER")
        # self.depth_left_shoulder = self.get_depth(body_part="LEFT_SHOULDER")
        # self.depth_right_elbow = self.get_depth(body_part="RIGHT_ELBOW")
        # self.depth_left_elbow = self.get_depth(body_part="LEFT_ELBOW")
        # self.depth_right_hip = self.get_depth(body_part="RIGHT_HIP")
        # self.depth_left_wrist = self.get_depth(body_part="LEFT_WRIST")
        # self.depth_right_wrist = self.get_depth(body_part="RIGHT_WRIST")
        # self.depth_left_hip = self.get_depth(body_part="LEFT_HIP")
        # self.depth_right_knee = self.get_depth(body_part="RIGHT_KNEE")
        # self.depth_left_knee = self.get_depth(body_part="LEFT_KNEE")
        # self.depth_right_ankle = self.get_depth(body_part="RIGHT_ANKLE")
        # self.depth_left_ankle = self.get_depth(body_part="LEFT_ANKLE")

        # print("self.depth_nose: ", self.depth_nose)
        # print("self.depth_right_shoulder: ", self.depth_right_shoulder)
        # print("self.depth_left_shoulder: ", self.depth_left_shoulder)
        # print("self.depth_right_elbow: ", self.depth_right_elbow)
        # print("self.depth_left_elbow: ", self.depth_left_elbow)
        # print("self.depth_right_hip: ", self.depth_right_hip)
        # print("self.depth_left_hip: ", self.depth_left_hip)
        # print("self.depth_right_knee: ", self.depth_right_knee)
        # print("self.depth_left_knee: ", self.depth_left_knee)
        # print("self.depth_right_ankle: ", self.depth_right_ankle)
        # print("self.depth_left_ankle: ", self.depth_left_ankle)
    ...
    #     # with open('waypoints.json') as f:
    #     with open('waypoints_v2.json') as f:
    #         self. waypoints_data = json.load(f)
    #         print(self. waypoints_data)
    ...
    #     lift_gripper_position = [self.robot.data['position'][0], self.sponge.data['position'][1]+ 0.15, self.robot.data['position'][2]]
    #     print(f"Raising gripper to safe height: {lift_gripper_position}")
    #     self.movegripper(lift_gripper_position)
    #     #env.Pend()

    #     # loading grasping position for sponge
    #     grasping_position = self. waypoints_data['grasping']['position']
    #     grasping_rotation = self. waypoints_data['grasping']['rotation']
    #     self.goto(grasping_position, grasping_rotation)
    ...
    #     # Use IK to position the gripper directly above the sponge (adjusting Z direction)
    #     fine_tune_position = [self.sponge.data['position'][0], self.sponge.data['position'][1] + 0.2, self.sponge.data['position'][2]]  # Position above the sponge at safe height
    #     print(f"Using IK to position above the sponge: {fine_tune_position}")
    #     self.movegripper(fine_tune_position)

    #     # Lower the gripper to reach the sponge
    #     lower_position = [self.sponge.data['position'][0], self.sponge.data['position'][1]+0.02, self.sponge.data['position'][2] + 0.05]
    #     print(f"Lowering gripper to reach sponge: {lower_position}")
    #     self.movegripper(lower_position)
    ...
    #     above_water_position = [-0.11, 0.8, 2.22]
    #     print(f"Move above water{above_water_position}")
    #     self.movegripper(above_water_position)
    #     #env.step(300)
    ...
    #     water_position = [-0.11, 0.58, 2.22]
    #     print(f"Lowering gripper to dip water {water_position}")
    #     self.movegripper(water_position)
    #     #env.step(300)
    ...
    #     #Move to head
    #     self.goto(self. waypoints_data['bedhead']['position'], self. waypoints_data['bedhead']['rotation'])
        
    #     #Move to tl 
    #     self.goto(self. waypoints_data['bedtl']['position'], self. waypoints_data['bedtl']['rotation'])
    #     #env.Pend()

    #     #Move to bathing area
    #     self.get_manikin_coordinates_depth()
    #     # bathing_start = self. waypoints_data['bathing1']['position']
    #     self. waypoints_data['bathing1']['position'][0] = self.target_nose[0]
    #     if self.target_nose[2] > 0.0:
    #         print("Go to Left side of bed")
    #         self. waypoints_data['bathing1']['position'][2] = 0.875
    #         self. waypoints_data['bathing1']['rotation'] = 90
    #     else:
    #         print("Go to Right side of bed")
    #         self. waypoints_data['bathing1']['position'][2] = -0.875
    #         self. waypoints_data['bathing1']['rotation'] = 270
    #     bathing_start = self. waypoints_data['bathing1']['position']

    #     # self.env.Pend()
    #     #bathing_start[0] =  nose[0]  

    #     direction, rot, dist = utils.move(self.robot.data['position'], bathing_start, self.robot.data['rotation'][1])
    #     if direction == "Left" :
    #         self.robot.TurnLeft(rot, 1)
    #     else:
    #         self.robot.TurnRight (rot, 1)
